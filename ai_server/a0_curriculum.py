...
def get_a0_group(index: int) -> dict | None:
    """Return the A0 group for a given lesson index, or None if out of range."""
    if 0 <= index < len(A0_GROUPS):
        return A0_GROUPS[index]
    return None


def get_group_letters(group: dict) -> list:
    """Return the list of items to practice in a group (letters or words)."""
    if group.get("type") == "blending":
        return [w["word"] for w in group.get("words", [])]
    return group.get("letters", [])
